...
exports.SimulatedProvider = exports.GeminiProvider = void 0;
/**
...
JSON Schema:
{
  ...
  "suggestedTests": ["test 1", "test 2"]
    ...
    async analyzeMedicalReport(title, content) {
...
JSON Schema:
{
  ...
  "recommendations": ["rec 1", "rec 2"]
    ...
    async generateHealthInsights(patientHistory) {
...
JSON Schema:
{
  ...
  "followUps": ["followup check 1", "followup check 2"]
    ...
    async recommendMedicines(symptoms, allergies) {
...
JSON Schema:
{
  ...
  "precautions": ["precaution 1", "precaution 2"]
    ...
    async calculateRiskScore(symptoms, vitals) {
...
JSON Schema:
{
  ...
  "details": "Explanation of clinical indicators justifying this risk estimation."
    ...
    async checkInteractions(med1, med2) {
...
class SimulatedProvider {
    ...
    async analyzeMedicalReport(title, content) {
        ...
        let keyMetrics = [
            ...
            { name: 'WBC Count', value: '7,500 /uL', status: 'Normal' }
        ];
        ...
        if (query.includes('cholesterol') || query.includes('lipid') || query.includes('ldl') || query.includes('hdl')) {
            keyMetrics = [
                { name: 'Total Cholesterol', value: '235 mg/dL', status: 'High' },
                { name: 'LDL Cholesterol', value: '158 mg/dL', status: 'High' },
                { name: 'HDL Cholesterol', value: '42 mg/dL', status: 'Normal' }
            ];
    ...
    async generateHealthInsights(_patientHistory) {
        return {
